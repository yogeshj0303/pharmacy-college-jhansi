...
<body>
    ...
    @foreach($TrainingDocument as $index => $Faculty)
        @if($index % 2 == 0)
            <div class="download-row"> <!-- Start a new row every 2 items -->
        @endif
        
        <div class="download-section">
            <a href="{{ asset($Faculty->document) }}" target="_blank" class="file-link">
    ...
    <select id="yearFilter" onchange="filterImages()">
        ...
        @foreach($PlacedStudentimages->unique('year') as $image)
            <option value="{{ $image->year }}">{{ $image->year }}</option>
        @endforeach   
    </select>

    <div class="images">
        @foreach($PlacedStudentimages as $image)
