...
<div class="row">
                ...
                <table id="documentTable" class="table table-bordered table-striped">
                    ...
                    <tbody>
                        ...
                        <td>{{ $documents->firstItem() + $index }}</td>
                            ...
                            <td>
                                <a href="{{ asset(''.$document->upload_pdf) }}" target="_blank" rel="noopener noreferrer">
                                    View Document
                                </a>
                            </td>
                            <td>
                                <button class="btn btn-primary btn-sm editDocument" data-id="{{ $document->id }}" data-title="{{ $document->title }}" data-document="{{ asset($document->upload_pdf) }}">Edit</button>
    ...
    <div class="col-md-5">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form id="documentForm" method="POST" action="{{ route('faculty-documents.store') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#editDocumentForm').on('submit', function (e) {
            ...
            $.ajax({
                url: '{{ route('faculty-documents.update', '') }}/' + $('#documentId').val(),
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        $(document).on('click', '.deleteDocument', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ route('faculty-documents.destroy', '') }}/' + id,
                        type: 'DELETE',
                        data: {
